              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-shrink-0 text-right">
                    <div className={`text-lg font-black ${medal.text} whitespace-nowrap tabular-nums leading-tight`}>
                      {getValue(entry, metric)}{unit && metric !== "time" && <span className="text-sm font-normal ml-0.5">{unit}</span>}
                    </div>
                    {index > 0 && (() => {
                      const leaderVal = getNumericValue(sorted[0], metric);
                      const gap = leaderVal - getNumericValue(entry, metric);
                      const gapStr = metric === "distance" ? `−${formatDistance(gap)} km`
                        : metric === "elevation" ? `−${formatNumber(gap)} m`
                        : metric === "time" ? `−${formatTime(gap)}`
                        : `−${gap}`;
                      return <div className="text-[10px] text-gray-500 mt-0.5">{gapStr} do lidera</div>;
                    })()}
                  </div>
                </div>
              </div>
